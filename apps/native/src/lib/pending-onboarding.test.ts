...
describe('onboarding recovery', () => {
  ...
  it('retains an older queued link and its operation id through partial replay', () => {
    storage.set('shelvr.pending.onboarding', JSON.stringify({
      operationId: 'legacy-operation', spaces: ['Recipes'], demoUrl: 'https://example.com',
    }));
  ...
  it('clears an older queued link only when that exact link was saved by the real demo', () => {
    storage.set('shelvr.pending.onboarding', JSON.stringify({
      operationId: 'legacy-operation', spaces: [], demoUrl: 'https://example.com',
    }));
  ...
  it('recovers the demo request and destination alongside survey and step progress', () => {
    setOnboardingProgress({ q1: ['X bookmarks'], q2: ['Inspiration'], spaces: ['Inspiration'], step: 5 });
    setPendingDemo({ url: 'https://example.com/design', destination: 'Inspiration' });
    setOnboardingProgress({ q1: ['X bookmarks'], q2: ['Inspiration'], spaces: ['Inspiration'], step: 6 });
    expect(getOnboardingProgress()).toEqual({
      q1: ['X bookmarks'], q2: ['Inspiration'], spaces: ['Inspiration'], step: 6,
