...
function validate(env: Record<string, string>) {
  runInNewContext(source, { require: requireConfig, module: { exports: {} }, process: { env }, URL });
}
...
describe("billing environment isolation", () => {
  ...
  it("rejects the old production-project Test Store key", () => {
    expect(() => validate({ EXPO_PUBLIC_REVENUECAT_TEST_KEY: "test_DzhYgoqsrxPykbPmjFpAMFbGBwn" })).toThrow(/Shelvr Development/);
  });

  it("requires the isolated test key in a development build", () => {
    expect(() => validate({ EAS_BUILD: "true", APP_VARIANT: "development" })).toThrow(/Shelvr Development/);
  });

  it("accepts the development backend and isolated Test Store", () => {
    expect(() => validate({ EAS_BUILD: "true", EXPO_PUBLIC_CONVEX_URL: "https://amicable-antelope-639.convex.cloud", EXPO_PUBLIC_REVENUECAT_TEST_KEY: testKey })).not.toThrow();
  });

  it("rejects a Test Store key in a production build", () => {
    expect(() => validate({ EAS_BUILD: "true", APP_VARIANT: "production", EXPO_PUBLIC_REVENUECAT_IOS_KEY: testKey })).toThrow(/appl_/);
  });

  it("rejects a development backend in a production build", () => {
    expect(() => validate({ EAS_BUILD: "true", APP_VARIANT: "production", EAS_BUILD_PLATFORM: "android", EXPO_PUBLIC_REVENUECAT_ANDROID_KEY: "goog_store", EXPO_PUBLIC_CONVEX_URL: "https://amicable-antelope-639.convex.cloud" })).toThrow(/production deployment URL/);
  });
});
