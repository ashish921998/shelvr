...
const item = (overrides: Partial<{ status: "processing" | "ready" | "failed"; fixtureKey?: string }> = {}) => ({
...
it('does not invite or claim a submission when analytics is opted out or disabled', async () => {
  ...
  expect(await feedbackAnalytics.submitFeedback('profile', 'Feedback')).toBe('unavailable');
  posthogMock.optedOut = false;
...
describe("canShowInvitation", () => {
  const eligible = (overrides: Partial<FeedbackInvitationState> = {}): FeedbackInvitationState => ({
    ...emptyInvitationState(),
    readyCount: 3,
    ...overrides,
  });
  const gate = (overrides: { now?: number; reviewPromptedAt?: number | null } = {}) => ({
  ...
  it("blocks below the threshold", () => {
    expect(
      canShowInvitation(eligible({ readyCount: 2 }), gate()),
    ).toBe(false);
  });
  ...
  it("never shows after a submission", () => {
    expect(canShowInvitation(eligible({ submitted: true }), gate())).toBe(false);
  });

  it("caps at two shows per account", () => {
    const shownTwice = eligible({ shownCount: FEEDBACK_MAX_INVITATIONS_PER_ACCOUNT });
  ...
  it("defers while the native review prompt is fresh", () => {
    const now = 1_000_000;
    const fresh = gate({ now, reviewPromptedAt: now - (FEEDBACK_REVIEW_PROMPT_COOLDOWN_MS - 1) });
    const cooled = gate({ now, reviewPromptedAt: now - FEEDBACK_REVIEW_PROMPT_COOLDOWN_MS });
...
describe("invitation state persistence", () => {
  it("round-trips per account and treats garbage as empty", () => {
    writeInvitationState("user-1", { readyCount: 5, shownCount: 1, lastShownAt: 42, submitted: false });
    ...
    kv.set("feedback.invitation.user-4", JSON.stringify({ readyCount: -3, shownCount: "x" }));
  ...
  it("marks submissions without touching other fields", () => {
    writeInvitationState("user-1", { readyCount: 3, shownCount: 1, lastShownAt: 9, submitted: false });
...
describe("parseInvitationState", () => {
  it("rejects non-object payloads", () => {
    expect(parseInvitationState('"just a string"')).toEqual(emptyInvitationState());
...
describe("feedbackAnalytics.submitFeedback", () => {
  it("queues the typed message with base properties on an explicit send", async () => {
    const result = await feedbackAnalytics.submitFeedback("home", "Love the feed");
...
describe("feedbackAnalytics automatic events", () => {
  it("carries surface metadata but never message content", () => {
    ...
    for (const call of posthogMock.capture.mock.calls) {
      ...
      expect(properties.ready_count === undefined || typeof properties.ready_count === "number").toBe(true);
    }
    expect(posthogMock.capture).toHaveBeenNthCalledWith(1, "feedback_invitation_shown", {
      surface: "home",
      ready_count: 3,
      environment: "development",
      analytics_version: 1,
    });
    expect(posthogMock.capture).toHaveBeenNthCalledWith(2, "feedback_invitation_dismissed", {
      surface: "home",
      environment: "development",
      analytics_version: 1,
    });
