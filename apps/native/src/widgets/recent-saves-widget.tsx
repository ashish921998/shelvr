...
const RecentSavesWidget = (
  props: RecentSavesWidgetProps,
  ...
  const gridCell = containerRelativeFrame({
    axes: "both",
    count: 2,
    spacing: 31,
  });
  const tile = (item?: WidgetSaveItem) => {
    ...
    return (
      <ZStack
        ...
        modifiers={[gridCell, clipShape("roundedRectangle", 12)]}
      >
        <Rectangle modifiers={[foregroundStyle(c.tile)]} />
        <VStack
          alignment="leading"
    ...
    >
      ...
      >
        ...
        >
          {!featured.imageUri ? (
            <Image
              systemName={kindIcon(featured.kind)}
              size={12}
              color={c.accent}
            />
          ) : null}
