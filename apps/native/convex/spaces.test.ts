...
describe("space creation", () => {
  ...
  it("bounds the name on create and rename with a message the client can show", async () => {
    ...
    expect((tooLong as ConvexError<string>).data).toBe(
      SPACE_NAME_TOO_LONG_MESSAGE,
    );
    ...
    expect((renamed as ConvexError<string>).data).toBe(
      SPACE_NAME_TOO_LONG_MESSAGE,
    );
  ...
  it("leaves a space named before the limit usable: replay finds it, edits keep it", async () => {
    ...
    const renamed = await t
      .mutation(api.spaces.updateSpace, {
        id: legacyId,
        name: legacyName + "z",
      })
      .then(
...
describe("filing corrections", () => {
  it("undoes acceptance back to a suggestion without dismissing it", async () => {
    ...
    await t.mutation(internal.spaces.setMembershipIntentsInternal, {
      ...ids,
      intents: [
        { kind: "web_search", label: "Find recipes", value: "pasta recipes" },
      ],
    });
  ...
  it("rejects corrections against another user's item or space", async () => {
    ...
    await expect(
      t.mutation(api.spaces.removeItemFromSpace, ids),
    ).rejects.toThrow("Item not found");
    await expect(t.mutation(api.spaces.addItemToSpace, ids)).rejects.toThrow(
      "Item not found",
    );
    await expect(
      t.mutation(api.spaces.undoAcceptSuggestion, ids),
    ).rejects.toThrow("Item not found");
  });
});
...
describe("denormalized space summary", () => {
  it("counts and previews follow add, accept, remove, item delete", async () => {
    const t = await entitledUser("user-a");
    const spaceId = await t.mutation(api.spaces.createSpace, {
      name: "Recipes",
    });
    ...
    expect(await readSpace(t, spaceId)).toMatchObject({
      ...
      suggestedCount: 1,
      previewItemIds: [d, c, b],
      suggestedPreviewItemIds: [s],
    });

    // Accepting moves it across buckets and to the front of the saved covers.
    expect(
      await t.mutation(api.spaces.acceptSuggestion, { itemId: s, spaceId }),
    ).toBe(true);
    expect(await readSpace(t, spaceId)).toMatchObject({
      savedCount: 5,
      suggestedCount: 0,
      previewItemIds: [s, d, c],
      suggestedPreviewItemIds: [],
    });
    ...
    expect(await readSpace(t, other)).toMatchObject({
      savedCount: 0,
      previewItemIds: [],
    });
    const joins = await t.run((ctx) =>
      ctx.db
        .query("spaceItems")
        .withIndex("by_item", (q) => q.eq("itemId", d))
        .collect(),
    );
  ...
  it("undo and dismiss move rows between buckets; setSpacesForItem withdraws suggestions", async () => {
    ...
    expect(await readSpace(t, spaceId)).toMatchObject({
      ...
      suggestedCount: 2,
      previewItemIds: [],
      suggestedPreviewItemIds: [x, y],
    });
    expect(
      await t.mutation(api.spaces.dismissSuggestion, { itemId: x, spaceId }),
    ).toBe(true);
    expect(await readSpace(t, spaceId)).toMatchObject({
      suggestedCount: 1,
      suggestedPreviewItemIds: [y],
    });
    ...
    expect(await t.mutation(api.spaces.acceptAllSuggestions, { spaceId })).toBe(
      0,
    );
  });
  ...
  it("deleting a space removes its joins and the row", async () => {
    ...
    expect(await readSpace(t, keep)).toMatchObject({
      savedCount: 1,
      previewItemIds: [a],
    });
  ...
  it("listSpaces reads counts and covers from the space row, not the joins", async () => {
    ...
    await t.run((ctx) =>
      ctx.db.patch(spaceId, {
        ...
        suggestedCount: 3,
        previewItemIds: [],
        suggestedPreviewItemIds: [withImage],
      }),
    );
  ...
  it("lists legacy rows without a summary from a bounded fallback, and backfills them", async () => {
    ...
    const result = await t.mutation(internal.spaces.backfillSpaceCounters, {
      batchSize: 1,
    });
    ...
    expect(
      jobs.filter((job) => job.name === "spaces:backfillSpaceCounters"),
    ).toHaveLength(1);
  ...
  it("backfill stops a batch when its read budget is spent and resumes at the next space", async () => {
    ...
    const withSummary = async () => {
      ...
      return rows
        .filter((row) => row.savedCount !== undefined)
        .map((row) => row._id);
    };
    ...
    const continuation = jobs.filter(
      (job) => job.name === "spaces:backfillSpaceCounters",
    );
    expect(continuation).toHaveLength(1);
    expect(continuation[0].args[0]).toMatchObject({
      cursor: first.cursor,
      readBudget: 8,
    });
    ...
    await t.run((ctx) =>
      ctx.db.patch(spaces[0], {
        savedCount: undefined,
        suggestedCount: undefined,
        previewItemIds: undefined,
        suggestedPreviewItemIds: undefined,
      }),
    );
    ...
    const forced = await t.mutation(internal.spaces.backfillSpaceCounters, {
      force: true,
    });
...
describe("steering spend controls", () => {
  it("blocks a burst of addItemToSpace past the steerItem capacity and rolls the add back", async () => {
    ...
    const joins = await t.run((ctx) =>
      ctx.db
        .query("spaceItems")
        .withIndex("by_item", (q) => q.eq("itemId", over))
        .collect(),
    );
    expect(joins).toHaveLength(0);
  });
  ...
  it("does not spend a steering token when the item is still processing", async () => {
    ...
    await t.mutation(api.spaces.addItemToSpace, {
      itemId: processing,
      spaceId,
    });
  ...
  it("files items for a non-entitled user but never schedules steering", async () => {
    ...
    await expect(
      t.mutation(api.spaces.createSpace, { name: "New" }),
    ).rejects.toThrow("Pro required");
  });
  ...
  it("rate-limits the recommendation pass on createSpace and on enabling dynamic", async () => {
    ...
    const replay = await t.mutation(api.spaces.createSpace, {
      name: "Space 0",
    });
    expect(replay).toBeDefined();
    await expect(
      t.mutation(api.spaces.createSpace, { name: "Space 10" }),
    ).rejects.toThrow();
    const spaces = await t.run((ctx) =>
      ctx.db
        .query("spaces")
        .withIndex("by_user", (q) => q.eq("userId", "rate-user"))
        .collect(),
    );
    ...
    expect(
      jobs.filter((job) => job.name === "ai:recommendForSpace"),
    ).toHaveLength(10);
  });
});
