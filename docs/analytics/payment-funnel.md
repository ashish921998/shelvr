...
2. **Survey mix over time** — `cancel_survey_submitted` grouped by `reason`, 100% stacked. The card asks once per account, so each person contributes at most one response. Reason-id → label mapping is kept in-repo (`apps/native/src/lib/cancel-survey.ts`) so PostHog data stays interpretable even if the card copy changes in a later build.
